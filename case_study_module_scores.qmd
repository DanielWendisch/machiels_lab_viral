...
```{r}
easypackages::libraries("Seurat","tidyverse","tidyseurat", "ggridges", "viridis", "ggh4x")
...
```{r}
aergerter_markers <- "../../raw_data/machiels_lab/viral/aergerter_2022_macs.xlsx" |> readxl::read_excel()
...
```{r}
vanneste_markers <- readxl::read_excel("../../raw_data/machiels_lab/viral/dataset_vanneste_et_al.xlsx")
...
just_mod_scores <- select(wide_mod_score_tbl,cMo__vanneste:pDCs__aergerter)
...
modul_score_means_tbl |>  ggplot(aes( x=module_scores,                                                                          harmony_cluster_8dims_rough,
...
#plot Z-score
dat |>  ggplot(aes( x=feat_group,                                                                          harmony_cluster_8dims_rough,
   fill=feat_zscore))+
  geom_tile()+
  scale_fill_viridis() +
  x_axis_text_90+
...
a <- c("darkgrey","black")[dat |> arrange (feat_group ) |> filter(harmony_cluster_8dims_rough ==0) |> pull(module_score)  |> as.numeric() %% 2+1]
...
#nested facets
dat |>  ggplot(aes( x=.feature,                                                                          harmony_cluster_8dims_rough,
   fill=feat_zscore))+
  geom_tile()+
  scale_fill_viridis() +
  x_axis_text_90+
